...
suite("secrets", () => {
  ...
  test("ignores values under minimum length threshold", async () => {
    ...
    const { checkpoints: secondCheckpoints } =
      await executeWithCheckpoints<string>(() => SecondComponent.run({}));
  ...
  test("handles secrets through component composition", async () => {
    ...
    const { checkpoints } = await executeWithCheckpoints<string>(() =>
      ParentComponent.run({ credentials }),
    );
  ...
  test("handles special characters in secrets", async () => {
    ...
    const { checkpoints } = await executeWithCheckpoints<string>(() =>
      SpecialCharComponent.run(config),
    );
  ...
  test("handles secrets in stream components", async () => {
    ...
    const { checkpoints, checkpointManager } = await executeWithCheckpoints(
      () => SecretStreamComponent.run({ apiKey }),
    );
    ...
    // Make sure we can iterate the stream
    let streamContent = "";
    if (
    ...
    // In streaming mode with secretOutputs:true, the individual stream tokens are masked
    // but accumulated in the final checkpoint
    expect(streamingFinal.output).toBeDefined();
