...
suite("checkpoint", () => {
  test("basic component test", async () => {
    ...
    const { result, checkpoints } = await executeWithCheckpoints(() =>
      SimpleComponent.run({ message: "world" }),
    );
  ...
  test("handles sequential execute calls within component", async () => {
    ...
    const ParentComponent = gensx.Component<{}, string>(
      "ParentComponent",
      async () => {
        const first = await SimpleComponent.run({ id: 1 });
        const second = await SimpleComponent.run({ id: 2 });
        const third = await SimpleComponent.run({ id: 3 });
  ...
  test("handles component children with object return", async () => {
    ...
    const ParentComponent = gensx.Component<{}, { a: string; b: string }>(
      "ParentComponent",
      async () => {
        const a = await ComponentA.run({ value: "first" });
        const b = await ComponentB.run({ value: "second" });
        return { a, b };
      },
    );
  ...
  test("handles nested component hierarchy", async () => {
    ...
    const ComponentB = gensx.Component<{ value: string }, string>(
      "ComponentB",
      async ({ value }) => {
        const inner = await ComponentC.run({ value: `${value}-inner` });
    ...
    const ComponentA = gensx.Component<{ value: string }, string>(
      "ComponentA",
      async ({ value }) => {
        const middle = await ComponentB.run({ value: `${value}-middle` });
  ...
  test("handles streaming components", async () => {
    ...
    const { result: nonStreamingResult, checkpoints: nonStreamingCheckpoints } =
      await executeWithCheckpoints(async () => {
        // Using type assertion to match our component's expected signature
        const result = await StreamingComponent.run({
          tokens: ["Hello", " ", "World"],
          stream: false,
        });
        return result;
    ...
    const {
      ...
      // Using type assertion for streaming mode
      const result = await StreamingComponent.run({
        tokens: ["Hello", " ", "World"],
        stream: true,
      });
    ...
    // Find the last checkpoint in the array
    ...
    if (lastCheckpoint) {
      ...
      if (lastCheckpoint.children && lastCheckpoint.children.length > 0) {
        // Get the StreamingComponent child
        const childComponent = lastCheckpoint.children.find(
          (child) => child.componentName === "StreamingComponent",
        );

        expect(childComponent).toBeDefined();
        if (childComponent) {
          expect(childComponent.output).toBe("Hello World");
          expect(childComponent.metadata?.streamCompleted).toBe(true);
        }
      }
    }
  });
  ...
  test("handles errors in streaming components", async () => {
    ...
    const {
      ...
      const result = await ErrorStreamingComponent.run({
        shouldError: true,
        stream: true,
      });
      return result;
    ...
    // Find the last checkpoint in the array
    const lastCheckpoint = errorCheckpoints[errorCheckpoints.length - 1];

    expect(lastCheckpoint).toBeDefined();
    if (lastCheckpoint) {
      // Check if there are children
      if (lastCheckpoint.children && lastCheckpoint.children.length > 0) {
        // Get the ErrorStreamingComponent child
        const childComponent = lastCheckpoint.children.find(
          (child) => child.componentName === "ErrorStreamingComponent",
        );

        expect(childComponent).toBeDefined();
        if (childComponent) {
          expect(childComponent.output).toBe("start");
          expect(childComponent.metadata?.error).toBe("Stream error");
          expect(childComponent.metadata?.streamCompleted).toBe(false);
        }
      }
    }
  });
  ...
  test("adds execution error to metadata when workflow throws", async () => {
    ...
    const { checkpoints, error } = await executeWorkflowWithCheckpoints(() =>
      ErrorComponent.run({ message: "Test error message" }),
    );
