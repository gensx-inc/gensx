...
export class CheckpointManager {
    ...
    constructor(opts) {
        this.nodes = new Map();
        this.orphanedNodes = new Map();
        this._secretValues = new Map(); // Internal per-node secrets
        this.currentNodeChain = []; // Track current execution context
        this.MIN_SECRET_LENGTH = 8;
        this.activeCheckpoint = null;
        this.pendingUpdate = false;
        this.version = 1;
        this.printUrl = false;
        this.havePrintedUrl = false;
        ...
        const apiBaseUrl = opts?.apiBaseUrl ?? process.env.GENSX_API_BASE_URL ?? config.api?.baseUrl;
        const consoleBaseUrl = opts?.consoleBaseUrl ??
        ...
        if (this.checkpointsEnabled && !this.org) {
            throw new Error("Organization not set. Set it via constructor options, GENSX_ORG environment variable, or in ~/.config/gensx/config. You can disable checkpoints by setting GENSX_CHECKPOINTS=false or unsetting GENSX_API_KEY.");
        }
    }
    ...
    handleOrphanedNode(node, expectedParentId) {
    ...
    checkOrphanTimeout(nodeId, expectedParentId) {
        setTimeout(() => {
            const orphans = this.orphanedNodes.get(expectedParentId);
            if (orphans?.has(this.nodes.get(nodeId))) {
                console.warn(`[Checkpoint] Node ${nodeId} (${this.nodes.get(nodeId)?.componentName}) still waiting for parent ${expectedParentId} after 5s`, {
                    node: this.nodes.get(nodeId),
                    existingNodes: Array.from(this.nodes.entries()).map(([id, node]) => ({
                        id,
    ...
    async writeCheckpoint() {
        ...
        try {
            ...
            const cloneWithoutFunctions = (obj) => {
            ...
            const maskedRoot = this.maskExecutionTree(treeCopy);
            const steps = this.countSteps(this.root);
            // Separately gzip the rawExecution data
            const compressedExecution = await gzipAsync(Buffer.from(JSON.stringify({
                ...maskedRoot,
                updatedAt: Date.now(),
            }), "utf-8"));
            const base64CompressedExecution = Buffer.from(compressedExecution).toString("base64");
            ...
            if (!this.traceId) {
                ...
                const url = join(this.apiBaseUrl, `/org/${this.org}/traces/${this.traceId}`);
            ...
            if (this.printUrl && !this.havePrintedUrl && response.ok) {
                const executionUrl = new URL(`/${this.org}/executions/${responseBody.executionId}?workflowName=${responseBody.workflowName}`, this.consoleBaseUrl);
    ...
    countSteps(node) {
        return node.children.reduce((acc, child) => acc + this.countSteps(child), 1);
    }
    maskExecutionTree(node) {
        // Mask props
        node.props = this.scrubSecrets(node.props, node.id);
        ...
        if (node.metadata) {
            node.metadata = this.scrubSecrets(node.metadata, node.id, "metadata");
        }
    ...
    isEqual(a, b) {
        ...
        if (!Array.isArray(a) && !Array.isArray(b)) {
            ...
            return (aKeys.length === bKeys.length &&
                aKeys.every((key) => this.isEqual(a[key], b[key])));
        }
        return false;
    }
    withNode(nodeId, fn) {
    ...
    registerSecrets(props, paths, nodeId) {
    ...
    collectSecretValues(data, nodeSecrets, visited = new WeakSet()) {
    ...
    scrubSecrets(data, nodeId, path = "") {
        return this.withNode(nodeId ?? "", () => {
            ...
            if (Array.isArray(data)) {
                return data.map((item, index) => this.scrubSecrets(item, nodeId, path ? `${path}.${index}` : `${index}`));
            }
    ...
    cloneValue(value) {
        ...
        return Object.fromEntries(Object.entries(value).map(([key, val]) => [key, this.cloneValue(val)]));
    }
    /**
     ...
     * of the execution.
     */
    addNode(partialNode, parentId) {
        const nodeId = generateUUID();
        const clonedPartial = this.cloneValue(partialNode);
        const node = {
            id: nodeId,
        ...
        if (parentId) {
            ...
            if (!this.root) {
                ...
                console.warn(`[Checkpoint] Multiple root nodes detected: existing=${this.root.componentName}, new=${node.componentName}`);
            }
        }
    ...
    completeNode(id, output) {
        const node = this.nodes.get(id);
        if (node) {
            node.endTime = Date.now();
            node.output = this.cloneValue(output);
            if (node.componentOpts?.secretOutputs &&
