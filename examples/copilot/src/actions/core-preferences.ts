...
export async function getCoreUserPreferences(
  userId: string,
): Promise<CorePreferences> {
  try {
    const searchClient = new SearchClient();
    const searchIndex = await searchClient.getNamespace(
    ...
    const result = await searchIndex.query({
      filters: [
        "Or",
        CORE_PREFERENCE_CATEGORIES.map((category) => [
          "preference",
          "Contains",
          category,
        ]),
    ...
    for (const row of result.rows) {
      ...
      if (preference.includes("name")) {
        ...
        preference.includes("communication") ||
        preference.includes("style")
      ) {
...
export async function getContextualPreferences(
  ...
  try {
    ...
    const relevantPreferences = await searchUserPreferences(
      userId,
      userPrompt,
      {
        limit,
    ...
    return relevantPreferences
      .filter(
        (pref) =>
          !CORE_PREFERENCE_CATEGORIES.some((core) =>
...
export function formatCorePreferencesForPrompt(
  corePrefs: CorePreferences,
): string {
