...
- Using the `GSXChatCompletion` component from `@gensx/openai` or `@gensx/anthropic` for fully automated tool calling
- Using the `OpenAIChatCompletion` or `AnthropicChatCompletion` components from `@gensx/openai` or `@gensx/anthropic` and following the same pattern as described in the [OpenAI](https://platform.openai.com/docs/guides/function-calling?api-mode=responses) and [Anthropic](https://docs.anthropic.com/en/docs/build-with-claude/tool-use/overview) docs
- Using tools with the [Vercel AI SDK](https://sdk.vercel.ai/docs/ai-sdk-core/tools-and-tool-calling) via the `@gensx/vercel-ai-sdk` package

This guide will focus on the first option: using `GSXTool` and the `GSXChatCompletion` component for fully automated tool calling.
        ...
        messages={[
          {
            role: "system",
            content:
              "You're a helpful, friendly weather assistant. You can use the tool `get_weather` to get the weather for a given location.",
          },
          {
            role: "user",
