...
| -------- | ------------------------------------------------------------------------------------------ |
| `<file>` | File to deploy. This should be a TypeScript/JavaScript file that exports a GenSX Workflow. |
...
1. Builds your workflow (if not already built)
2. Uploads it to GenSX Cloud
3. Creates or updates the deployment
4. Sets up any environment variables specified

After successful deployment, your workflow will be available for remote execution via the GenSX Cloud dashboard or through the `gensx run` command.
...
- After deployment, you can manage your workflows from the GenSX Cloud dashboard
