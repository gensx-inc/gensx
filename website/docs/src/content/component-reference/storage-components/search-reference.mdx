...
| `deleteByFilter` | `Filter`         | `undefined` | Filter to match documents for deletion      |
...
| Parameter           | Type                  | Default     | Description                              |
| ------------------- | --------------------- | ----------- | ---------------------------------------- |
| `rankBy`            | `RankBy`              | `undefined` | Vector, text, or attribute-based ranking |
| `topK`              | `number`              | `undefined`        | Number of results to return              |
| `includeAttributes` | `boolean \| string[]` | `['id']`    | Include all attributes or specified ones |
| `filters`           | `Filter`              | `undefined` | Metadata filters                         |
| `aggregateBy`       | `AggregateBy`         | `undefined` | Aggregate results by specified fields    |
| `consistency`       | `Consistency`         | `undefined` | Consistency level for reads              |
  ...
  rows: [
    {
      ...
      $dist: 0.87,            // Distance score (lower is more similar for most metrics)
      vector?: number[],      // Original vector (if includeAttributes includes vectors)
      text: "Document content",     // Document attributes
      category: "article",
