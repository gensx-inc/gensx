...
const blogWritingWorkflow = gsx.workflow("BlogWritingWorkflow", BlogWriter);
const result = await blogWritingWorkflow.run({ prompt: "Write a blog post about AI developer tools" });
```
