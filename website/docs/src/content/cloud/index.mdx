...
GenSX Cloud provides everything you need to ship production grade agents:
...
GenSX Cloud provides serverless infrastructure optimized for agents and LLM workflows:
...
- **Millisecond-level cold starts**:

Serverless deployments are billed pay-second, with 50,000 seconds included per month in the free tier for individuals.
...
Each workflow is available via APIs for synchronous and asynchronous invocation:

```
// For synchronous and streaming calls:
https://api.gensx.com/org/{orgName}/projects/{projectName}/environments/{environmentName}/workflows/{workflowName}/run
...
const Workflow = ({ userInput, threadId }) => (
  <BlobProvider>
...
📋 Available workflows:
- MyWorkflow: http://localhost:1337/local/my-app/default/workflows/MyWorkflow/run
...
- **Projects**: A collection of related workflows that are deployed together, typically corresponding to a codebase or application. Projects help you organize and manage your AI components as cohesive units.
...
- **Environments**: Sub-groupings within a project that allow you to deploy multiple instances of the same workflows with different configuration. This supports the common development pattern of separating dev, staging, and production environments.
