...
3. **API Response**: The execution ID from API responses can be used to view traces in the console
...
// [GenSX] View execution at: https://app.gensx.com/your_org/executions/your_execution_idf
```

### Viewing the flame graph
...
tGenSX provides flexible options for configuring and organizing traces for the GenSX Cloud serverless platform, local development, and any other deployment platform like vercel, cloudflare and AWS.

### Tracing GenSX Cloud workflows

When running workflows on GenSX Cloud, tracing is automatically configured:
...
To enable tracing for workflows deployed outside of GenSX Cloud (like AWS Lambda, Google Cloud Functions, etc.), you need to set several environment variables:
...
For local development, tracing configuration is automatically inferred from:

1. The `gensx.yaml` file in your project root
2. Your local configuration managed by the `gensx` CLI in `~/.config/gensx/config`
3. Optionally the `GENSX_ENVIRONMENT` environment variable to separate local traces from other environments
...
GenSX allows you to organize traces by environment (development, staging, production) to keep your debugging data well-structured:
...
Secrets are automatically propagated through the component hierarchy:

```tsx
const ParentComponent = gensx.Component(
  "ParentComponent",
  async ({ apiKey, query }) => {
    // apiKey is marked as secret
    const results = await ChildComponent.run({
      query,
      headers: { authorization: `Bearer ${apiKey}` }
    });

    // apiKey is also scrubbed here in the trace,
    // even though ChildComponent doesn't explicitly mark it
    return processResults(results, apiKey);
  },
  {
    secretProps: ["apiKey"]
  }
...
Secrets remain scrubbed throughout the execution chain, even if they're transformed or embedded in different objects.
