...
In some cases, you may need to run a sub-workflow within a larger workflow. You can do this using both `Component.run()` and `gensx.execute()`:
...
This allows you to work with a component in TypeScript without having to drop into the JSX layer. When used inside a component, `gensx.execute()` and `Component.run()` preserve the current context and maintain the component hierarchy so the calls will integrate naturally with the rest of the workflow.
