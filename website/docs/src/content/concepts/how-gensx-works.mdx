...
GenSX also allows you to execute sub-workflows from within a component using both `gensx.execute()` and `Component.run()`.
...
When used inside a component, these operations preserve the current context and maintain the component hierarchy so they will integrate naturally with the rest of the workflow. The result is automatically resolved to a plain JavaScript value, allowing you to work with it directly without additional JSX syntax.
