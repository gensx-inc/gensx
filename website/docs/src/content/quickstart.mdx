...
1. install / acquire CLI
2. log in -- add note about skipping this if you don't want to save your runs
3. Add section on tracing and visualization. -- add notes that if you skipped logging into the CLI, you can still do so to view the traces.
...
You can install the `gensx` CLI globally using your package manager of choice:

```bash
npm i -g gensx
```

Or if you prefer to avoid a global installation, you can prefix commands with `npx`:

```bash
npx gensx
```

Now run `gensx login` or `npx gensx login`. This will save your traces to the cloud so you can visualize and debug workflow executions, but this step is optional and you can skip it for now if you prefer.
...
Now you're ready to create a new project! Return to your terminal for the next steps.
...
The component is executed through `gsx.Workflow.run()`, which processes the JSX tree from top to bottom. In this example:
...
const WriteArticle = gsx.Component<{ topic: string; research: string }, string>(
  "WriteArticle",
...
const ResearchAndWrite = gsx.Component<{ topic: string }, string>(
  "ResearchAndWrite",
  ({ topic }) => (
    <OpenAIProvider apiKey={process.env.OPENAI_API_KEY}>
...
const workflow = gsx.Workflow("ResearchAndWriteWorkflow", ResearchAndWrite, {
  printUrl: true,
});
```
...
const Writer = gsx.StreamComponent<WriterProps>(
  "Writer",
...
const workflow = gsx.Workflow("ResearchAndWriteWorkflow", ResearchAndWrite, { printUrl: true });
