...
GenSX provides a local development with local REST APIs that match the shape of workflows deployed to GenSX Cloud. Run the dev server from the CLI:
...
GenSX Cloud provides serverless deployment with zero configuration:
...
gensx run ResearchAndWriteWorkflow --input '{"topic":"quantum computing"}' --wait --output result.json
```
...
GenSX Cloud provides built-in storage solutions with zero configuration:
...
const ChatWithMemory = gensx.Component<{ userInput: string, threadId: string }, string>(
  "ChatWithMemory",
