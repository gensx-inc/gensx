...
Components can be nested to create multi-step workflows with each component's output being passed through a child function. For example, let's define two components:
...
While this is nice, the real power of this comes when you expand or refactor your workflow. Now you could easily add an `<Editor>` component to the workflow that streams the response to the user with minimal changes. There's no extra plumbing needed besides removing the `stream={true}` prop on the `Writer` component.
