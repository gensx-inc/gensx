...
For many workflows, you'll need to get structured output from LLMs. GenSX makes it easy to get structured, type-safe output from LLMs. This guide showcases how to use structured outputs with OpenAI models but you can follow a similar pattern with other LLM providers.

All GenSX components include an input and output type. The most common use case is to use structured outputs to ensure that the response from the LLM matches the format of the component's output type.
