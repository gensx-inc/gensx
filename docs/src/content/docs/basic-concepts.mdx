...
Components can return a primitive, an object, or other components. Their outputs can be used by other components or returned as the result of the workflow.
...
Here's an example of a component that returns another component:
...
Components can also return structured objects containing. The pattern works with array operations like `map` for processing items in parallel.
...
1. **Components** (`gsx.Component`) - Components that produce values and can handle both synchronous and asynchronous operations
2. **Streaming Components** (`gsx.StreamingComponent`) - Components designed to handle responses from LLMs, working in both streaming and non-streaming contexts

The power of a `StreamingComponent` is that a single implementation can be used interchangeably in both streaming and non-streaming contexts without any code changes - you just toggle the `stream` prop:
...
Components are executed using `gsx.execute()`, which processes the JSX tree from top to bottom while executing components in parallel as much as possible.

```tsx
const result = await gsx.execute(<MyComponent input="some data" />);
```

You'll often use `gsx.execute()` to run a top level workflow as shown above, but you can also use it to run a sub-workflow within a component. This allows you to work with a component in TypeScript without having to drop into the JSX layer. This is especially useful for more complex operations like map/reduce:
...
const AnalyzeReviews = gsx.Component<AnalyzeReviewsProps, AnalyzeReviewsOutput>(
  async ({ reviews }) => {
    ...
    const topics = await Promise.all(
      reviews.map((review) => gsx.execute(<GetTopic review={review} />)),
    );
...
const UserGreeting = gsx.Component(() => {
